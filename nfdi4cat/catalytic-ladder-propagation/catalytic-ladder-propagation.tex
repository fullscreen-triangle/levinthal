...
\large Federated Retrieval of Heterogenous Sources Catalytic Data through Translated Graph Verdicts }
...
\begin{tabular}{@{}llrr@{}}
\toprule
Form & Distinguishing line & Result & Elapsed\\
\midrule
A (object list)  & \texttt{\dots\ rh:chebi ?a , ?o .} & $2$ & $2.7$\,s\\
B (two patterns) & \texttt{\dots\ rh:chebi ?a .}\ \ \texttt{\dots\ rh:chebi ?o .} & $397$ & $37.3$\,s\\
\bottomrule
\end{tabular}
\end{center}

The elapsed times are reported because they are the one piece of diagnostic
evidence the measurement carries. Form~A returns a smaller answer roughly
fourteen times faster, which is the signature of a route that explored less of
the store rather than of one that explored the same store and found less. We
draw no conclusion from it --- timing against a shared public service is
confounded by load, caching and concurrent queries --- but a reader
reconstructing the observation should have the numbers.
...
\subsection{What running the suite changed}
\label{sec:whatchanged}

Four episodes altered what we believed, and each is load-bearing enough to
state rather than to absorb silently.

\paragraph{A hand-computed floor that was wrong, in the way the paper warns
about.} Our first expectation for the small graph of \Cref{sec:res-a} was
$\flo=2$, taken from the lightest edge incident to a vertex. The computed
value is $5$, attained by the \emph{region} $\{a,b\}$ rather than by either
vertex alone: separating $a$ costs $\min(2+5,\,2+3)=5$. The error is exactly
the one \Cref{rem:notedge} and \Cref{prop:region} exist to forbid --- reading
a separation cost as an edge weight --- and we had made it while writing the
section that forbids it. The check now records the expected value as a
separation cost and carries the reasoning in a comment.

\paragraph{An adversarial instance that does not do what its name suggests.}
We included a graph whose smallest edge is $10^{-12}$ expecting the floor to
follow it down. It does not: the floor is $1+10^{-12}$, because that tiny edge
lies inside every cut that would use it, and a cheap edge inside an expensive
cut does not make the cut cheap. The instance is retained, with its
expectation corrected, because it is a sharper illustration of
\Cref{rem:notedge} than the one we designed.

\paragraph{A one-sided sweep that looked like a refutation.}
\Cref{rem:saturation-found} records this in full. The short form: a sweep
varying only the product-end occupancy reached cases (a) and (c) of
\Cref{thm:trichotomy} and never case (b), which presents as a theorem with an
unreachable branch. The theorem was not at fault; the sweep was, and repairing
it produced \Cref{prop:saturation}, which we had not anticipated and would not
have found by inspection.

\paragraph{A check that was scored when it should have been excluded.} The
permutation check of \Cref{sec:protocol} was initially counted among the
passes. It cannot fail --- \Cref{eq:compose} is symmetric in its arguments ---
so counting it inflated the score by one and told the reader nothing. It is
now excluded and reported as excluded. We mention it because the difference
between $58$ checks and $57$ scored checks is precisely the difference between
a suite that reports what it measured and one that reports how many boxes were
green.

\begin{remark}[Two figures that would have hidden a result]
\label{rem:figures-honest}
Two panels were redrawn after they were first plotted.
\Cref{fig:medium}(c) originally showed a single depletion sweep on axes that
made the flooding branch look like an unbounded descent; the ceiling of
\Cref{prop:saturation} was off-scale and invisible. \Cref{fig:ladder}(a) was
first drawn without a legend distinguishing the four composition laws, which
made the comparison --- the whole content of the panel --- unreadable. In both
cases the figure was drawn from correct numbers and still conveyed the wrong
thing, which is a reminder that a computed panel is not automatically an
honest one.
\end{remark}
...
All experiments are in \texttt{https://github.com/fullscreen-triangle/levinthal/tree/main/nfdi4cat/catalytic-ladder-propagation/validation/}. Running
