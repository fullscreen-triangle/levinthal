...
\citep{denisov2005,rittle2010,jung2011,shahrokh2012}.  The accepted minimal mechanism
...
axial ligand identity, and $s$ tracks net spin parity~\citep{bren2015}.
...
O--O heterolysis ($\Delta\depth = \ln 2$, Paper~5~\citep{spolitak2005,mak2007}), and C--H activation
($\Delta\depth = 0.65$, Paper~6~\citep{groves1985}) are taken directly from the earlier papers.
...
($\Delta\depth = 7.60$)~\citep{marcus1985}. This tunneling rate is separated from the intrinsic
...
\caption{\textbf{Poincar\'e return time of the catalytic orbit.}
...
(\textbf{D}) Three-dimensional Poincar\'e return map in
...
\begin{enumerate}
  ...
  \item \textbf{Paper 3}: Resting and substrate-bound states — the 1$\to$2
    transition with $\Delta\depth = 0.92$ and the $+120$\,mV redox shift~\citep{sligar1979}.
...
changes; radical character~\citep{guengerich1990}) is precisely seven.  Fewer states would require
