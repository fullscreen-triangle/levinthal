...
only a taxonomy for the 57 human CYP isoforms~\citep{Nelson2004} but also a robust recovery
...
Public P450 databases (UniProt, PharmVar, CYPalleles, CYP450 Engineering DB~\citep{Preissner2010})
collectively contain hundreds of thousands of annotated sequences~\citep{Gaedigk2018}.  However,
...
3-ary tree.  The tree provides hierarchical error correction~\citep{Hamming1950}: if the lower
...
Each trit carries $\log_2 3 \approx 1.585$~bits of information~\citep{Shannon1948}.  At depth $k$,
...
We model recovery accuracy using an exponential saturation model~\citep{Hamming1950}:
\begin{equation}
...
CYP101A1 vs.\ CYP3A4~\citep{Denisov2005}).  Using the exponential recovery model:
\begin{equation}
...
alleles, and $\sim$310 total alleles across 4 major genes~\citep{Gaedigk2018}.  Ternary depth
