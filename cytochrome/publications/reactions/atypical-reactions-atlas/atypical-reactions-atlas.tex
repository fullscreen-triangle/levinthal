...
C=O bond~\citep{Guengerich2018,Ortiz_de_Montellano2015}.  The canonical hydrogen-atom transfer (HAT) rebound mechanism
accounts for aliphatic and benzylic hydroxylation~\citep{Groves2006,Shaik2010}.  However, the active
...
\item[Desaturation] Two sequential HAT steps convert a vicinal $\text{C}_\alpha$--H
  and $\text{C}_\beta$--H into a double bond instead of a hydroxylated product~\citep{Bhatt2013}.
...
\item[NIH shift] The arene oxide from epoxidation undergoes a spontaneous
  ...
  experiments, yielding a phenolic product~\citep{Korzekwa1989}.
\item[Nucleophilic O-atom transfer] At aldehyde carbonyls, the iron-oxo
  ...
  Baeyer--Villiger-like mechanism~\citep{Watanabe2001}.
\item[Carbene insertion] Engineered P450s (e.g.\ CYP119 variants) can accept
  diazo compounds to form an iron-carbene that inserts into C--H or X--H bonds~\citep{Coelho2013,Wang2014}.
\end{description}
...
Because no hydrogen is transferred in the rate-limiting step, $\text{KIE} = 1.0$~\citep{Korzekwa1989,Shaik2010}.
...
and steroidogenic P450s~\citep{Watanabe2001}.  Since no C--H bond is broken, $\text{KIE} = 1.0$.
...
Table~\ref{tab:delta_m} and Figure~\ref{fig:panel06_ordering} show that $\Delta M$
values increase monotonically from NIH shift to desaturation, and rates
decrease accordingly~\citep{Guengerich2018}:
\begin{equation}
