...
\label{sec:what-results-establish}

Three of the outcomes in \cref{sec:results} carry more weight than their
pass/fail status suggests, and one changes how a claim in this paper should be
read. We set them out here rather than leaving them to be inferred.

\subsection{Non-consumption is independently encoded}

The Reactome result is the strongest external check in the suite, and it is
not a check we designed. Of $990$ human reactions retrieved, $363$ carry an
annotated catalytic activity, and in none of them does any species appear on
both the input and the output side. The curators placed catalysis in a slot
disjoint from stoichiometry.

What makes this evidential rather than circular is that the annotation
predates the framework and was made on mechanistic grounds by people with no
stake in \cref{thm:not-consumed}. A catalyst that were a reactant regenerated
downstream would appear identically on both sides and would be recorded that
way; the schema permits it, and it does not occur. The database is therefore an
independent witness to the structural claim that the catalyst is not
stoichiometric---which on the present account is not a convention but a
consequence of \cref{cor:provision-free}.

\subsection{The reciprocal law is excluded without reference to fit}

Discriminating between rate laws usually means comparing residuals, which
invites the objection that the comparison was tuned. The dynamic-range
argument avoids this entirely. Observed catalytic efficiencies span $5.62$
decades. A law of the form $k \propto 1/(\dC\tau)$, with $\dC$ ranging over the
$1$--$6$ apertures documented for these mechanisms, can span at most
$\log_{10} 6 = 0.78$ decades whatever $\tau$ is chosen, because $\tau$ enters
as a common factor and cancels from the ratio. The observed range exceeds the
representable range by a factor of seven in the exponent.

This is a structural exclusion. It does not depend on the values in
\cref{eq:rate}, on $\nufloor$, on $\delta$, or on the quality of any fit; it
depends only on the arithmetic of what a reciprocal law can represent. The
exponential form is required by the magnitude of the phenomenon, as
\cref{rem:not-reciprocal} argues on physical grounds and the data confirm on
arithmetic ones.

\subsection{The inhibition dichotomy is a joint constraint, not a definition}

\Cref{thm:inhibition} could be dismissed as terminological: one might suspect
that ``competitive'' and ``mechanism-based'' are simply the names given to the
two cells, so that finding only two occupied is guaranteed. The permutation
control settles this. The two axes---whether inactivation requires turnover,
and whether activity is recovered on dilution---are established by different
experiments and neither is defined in terms of the other. Both take both
values across the sample. Shuffling the reversibility labels fills, on
average, $3.999$ of the four cells; the observed restriction to two arises
with $p = 4\times10^{-5}$.

The forbidden cells are the informative part. A turnover-dependent yet fully
reversible inhibitor, or a turnover-independent yet irreversible one acting at
the catalytic contact, would each occupy a cell the conjunction
$\prov>0 \wedge \text{release}$ has no room for. Neither is observed among the
nineteen documented cases examined.

\subsection{One claim is weaker than we first stated it}

The cofactor-persistence prediction of \cref{part:origins} is not supported by
the evidence we were able to bring to it, and we prefer to say so plainly. Our
test found the predicted delocalised motifs in $36.6\%$ of KEGG enzyme entries,
which in isolation looks like confirmation. The matched control---a list of
common metabolites of equal length, searched in the same fields---returned
$30.3\%$. A ratio of $1.21$ is not a separation.

The reason is a property of the data rather than of the prediction: KEGG
enzyme entries carry no cofactor field, so the search necessarily ran over
substrate, product and comment text, where every participant appears and
where frequency of mention reflects corpus composition. The prediction may
well be correct; this test cannot tell us. It is reported as
non-discriminating and excluded from the score, and the corresponding
limitation is recorded in \cref{sec:limitations}.

We draw attention to it because the alternative---reporting the $36.6\%$ and
omitting the control---would have produced a table in which every entry
passed, and a reader could not have told the difference.
