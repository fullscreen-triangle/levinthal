...
\begin{figure}[H]
\centering
\fbox{\parbox{0.9\columnwidth}{\centering\vspace{1.5em}\textbf{Figure Placeholder: Folding Probe Architecture}\\\vspace{0.5em}Sequence encoder (transformer blocks with LoRA adapters) mapping amino acid tokens to S-entropy address contributions, feeding into trajectory decoder that generates trit-by-trit trajectory completion. Arrow labeled ``probe'' from input to encoder, arrow labeled ``complete'' from encoder to decoder output.\vspace{1.5em}}}
\caption{\textbf{Folding probe architecture.} The encoder maps each residue to its S-entropy contribution; the decoder generates the trajectory completion path to the native structure.}
\label{fig:folding_probe}
\end{figure}
...
\begin{figure}[H]
\centering
\fbox{\parbox{0.9\columnwidth}{\centering\vspace{1.5em}\textbf{Figure Placeholder: Design Probe as Inverse Compilation}\\\vspace{0.5em}Left: target address in S-entropy space (point on the conservation manifold). Center: inverse trajectory decoder generating residue-by-residue sequence. Right: verification through forward trajectory completion confirming convergence to the target address. Arrows show the inverse then forward path.\vspace{1.5em}}}
\caption{\textbf{Design probe architecture.} The inverse trajectory decoder generates a sequence from a target S-entropy address; the forward trajectory verification confirms correctness.}
\label{fig:design_probe}
\end{figure}
