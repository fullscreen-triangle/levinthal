...
\begin{figure*}[!htbp]
    \centering
    \includegraphics[width=\textwidth]{figures/panel_1_vibrational_hierarchy.png}
    \caption{\textbf{Protein Vibrational Mode Hierarchy.}
    (\textbf{A})~Frequency spectrum of a 150-residue globular protein showing all $\sim$3600 normal modes. Modes are coloured by hierarchical level: Level~1 bond vibrations (red, $>$2000~cm$^{-1}$), Level~2 amide modes (orange, 1200--1700~cm$^{-1}$), Level~3 secondary structure modes (green, 100--1200~cm$^{-1}$), Level~4 domain breathing modes (blue, 1--100~cm$^{-1}$), Level~5 global modes (purple, $<$1~cm$^{-1}$).
    (\textbf{B})~Hierarchical partition tree showing the correspondence between vibrational levels and partition depths. Each level refines the partition established by higher-frequency levels, producing a nested trie structure.
    (\textbf{C})~Timescale separation across the five levels. The ratio $\omega_{\mathrm{max}}/\omega_{\mathrm{min}}$ within each level is moderate ($\sim$2--5$\times$), while the ratio between adjacent levels spans one to two orders of magnitude, ensuring clean hierarchical separation.
    (\textbf{D})~Amide I band shapes for three secondary structure types: $\alpha$-helix (single peak at $\sim$1650~cm$^{-1}$), $\beta$-sheet (split band at $\sim$1630 and $\sim$1680~cm$^{-1}$), and random coil (broad peak at $\sim$1645~cm$^{-1}$). These distinct spectral signatures enable Level~3 partition refinement.}
    \label{fig:vibrational_hierarchy}
\end{figure*}
...
\begin{figure*}[!htbp]
    \centering
    \includegraphics[width=\textwidth]{figures/panel_2_amino_acid_space.png}
    \caption{\textbf{Amino Acid S-Entropy Space.}
    (\textbf{A})~All 20 amino acids embedded in three-dimensional S-entropy space $\Sspace = [0,1]^3$. Points coloured by chemical class: nonpolar (blue), polar uncharged (green), positively charged (red), negatively charged (orange), aromatic (purple). The class-dependent clustering emerges from normalized physicochemical properties alone.
    (\textbf{B})~Chemical family clustering at trit depth 3. The unit cube is partitioned into 27 cells ($3 \times 3 \times 3$). Shaded cells contain amino acids; colour indicates the dominant chemical class within each cell. Nonpolar residues concentrate in the high-$\Sk$, low-$\Se$ corner; charged residues in the low-$\Sk$, high-$\Se$ corner.
    (\textbf{C})~Pairwise Euclidean distance matrix for all 20 amino acids in $\Sspace$, sorted by chemical class. Block diagonal structure confirms within-class proximity.
    (\textbf{D})~Resolution depth analysis: number of uniquely resolved amino acid pairs as a function of trit depth $k$. All 190 pairs are resolved by $k = 9$.}
    \label{fig:amino_acid_space}
\end{figure*}
...
\begin{figure*}[!htbp]
    \centering
    \includegraphics[width=\textwidth]{figures/panel_3_protein_trie.png}
    \caption{\textbf{Multi-Scale Protein Trie Structure.}
    (\textbf{A})~Schematic of the nested trie. The outermost level (global modes) classifies proteins by fold class ($\alpha$, $\beta$, $\alpha/\beta$, etc.). Intermediate levels refine by domain and secondary structure. The innermost level resolves individual residues.
    (\textbf{B})~A sample protein address at four hierarchical levels, showing how each level's S-entropy coordinates contribute trits to the full address.
    (\textbf{C})~Address comparison between two structurally similar proteins (same fold, different sequence) showing shared prefixes at global and domain levels diverging at residue level.
    (\textbf{D})~Address comparison between two structurally dissimilar proteins (different fold class) showing divergence at the global level (first few trits).}
    \label{fig:protein_trie}
\end{figure*}
...
\begin{figure*}[!htbp]
    ...
    \caption{\textbf{Validation Framework.}
    (\textbf{A})~Amino acid resolution profile: number of uniquely resolved pairs vs.\ trit depth. The curve reaches 190/190 at depth 9.
    (\textbf{B})~Chemical family clustering at depth 3 in $\Sspace$. The five biochemical classes occupy distinct regions without chemical labels being encoded.
    (\textbf{C})~Predicted identification speedup as a function of database size $N$. The categorical approach (flat line at $O(k)$) versus sequence alignment ($O(NL)$, diagonal line).
    (\textbf{D})~Folding trajectory completion: number of hierarchical steps vs.\ protein length $\Nres$, showing logarithmic scaling.}
